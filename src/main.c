...
int main() {
  ...
  background_music = LoadMusicStream("assets/bg-music.mp3");
  ...
  while (!WindowShouldClose()) {
      ...
      if (x >= 0 && x < BOARD_SIZE && y >= 0 && y < BOARD_SIZE) {
        ...
        if (selected_tile.x < 0) {
          ...
          if (are_tiles_adjacent(selected_tile, current_tile)) {
            swap_tiles(selected_tile.x, selected_tile.y, current_tile.x,
                       current_tile.y);
            if (find_matches()) {
              resolve_matches();
            } else {
    ...
    DrawRectangle(grid_origin.x, grid_origin.y, BOARD_SIZE * TILE_SIZE,
                  BOARD_SIZE * TILE_SIZE, Fade(DARKGRAY, 0.50f));

    for (int y = 0; y < BOARD_SIZE; y++) {
      for (int x = 0; x < BOARD_SIZE; x++) {
        Rectangle rectangle = {grid_origin.x + (x * TILE_SIZE),
        ...
        if (board[y][x] != ' ') {
          DrawTextEx(
              font, TextFormat("%c", board[y][x]),
              (Vector2){rectangle.x + 15, rectangle.y + 12 - fall_offset[y][x]},
              20, 1.0f, matched[y][x] ? PURPLE : WHITE);
        }
      }
    }
    ...
    if (selected_tile.x >= 0) {
      DrawRectangleLinesEx(
          (Rectangle){grid_origin.x + (selected_tile.x * TILE_SIZE),
                      grid_origin.y + (selected_tile.y * TILE_SIZE), TILE_SIZE,
                      TILE_SIZE},
          2.0f, PURPLE);
    }

    DrawTextEx(font, TextFormat("SCORE: %d", score), (Vector2){20, 20},
               24 * score_scale, 1.0f, PURPLE);

    for (int i = 0; i < MAX_SCORE_POPUPS; i++) {
      if (score_popups[i].active) {
        Color c = Fade(MAGENTA, score_popups[i].alpha);
